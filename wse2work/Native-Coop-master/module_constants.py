...
num_coop_skills = 42  # Warband has 42 skill slots (0-41)
num_coop_attrs  = 4
num_coop_profs  = 7
...
slot_coop_char_level = 31

# trp_temp_troop slots for party screen snapshot (diff on close)
slot_coop_party_snap_count = 40          # number of stacks snapshotted
slot_coop_party_snap_begin = 41          # 41-99: max 20 stacks * 3 slots each (troop, size, delta)
slot_coop_party_snap_stride = 3

# trp_temp_troop slots for character screen snapshot (diff on close)
slot_coop_char_snap_attr_begin  = 100  # 100-103: attributes (STR, AGI, INT, CHA)
slot_coop_char_snap_skill_begin = 104  # 104-145: skills (0-41)
slot_coop_char_snap_prof_begin  = 146  # 146-152: proficiencies (0-6)
slot_coop_char_snap_attr_pts    = 153
slot_coop_char_snap_skill_pts   = 154
slot_coop_char_snap_prof_pts    = 155

# Inventory snapshot slots in trp_temp_troop (for close-diff)
# Equipment: 10 item slots + 10 modifier slots = 20
# Bag: 96 item slots + 96 modifier slots = 192
# Total: 212 slots, range 160-371
slot_coop_inv_snap_equip_item_begin = 160   # 160-169: equipment item IDs (slots 0-9)
slot_coop_inv_snap_equip_mod_begin  = 170   # 170-179: equipment modifiers (slots 0-9)
slot_coop_inv_snap_bag_item_begin   = 180   # 180-275: bag item IDs (slots 10-105)
slot_coop_inv_snap_bag_mod_begin    = 276   # 276-371: bag modifiers (slots 10-105)

# trp_temp_array_b slots for merchant trade snapshot (diff on close)
...
# trp_temp_array_c slots for local battle pre-snapshot (count troops before fight)
slot_coop_local_snap_count      = 0     # number of stacks snapshotted
slot_coop_local_snap_begin      = 1     # 1-60: max 20 stacks * 3 (troop_id, size_before, 0)
slot_coop_local_snap_stride     = 3
...
# Merchant type constants (index into center party slots)
coop_merchant_type_weaponsmith   = 0   # slot_town_weaponsmith (21)
coop_merchant_type_armorer       = 1   # slot_town_armorer (22)
coop_merchant_type_horse         = 2   # slot_town_horse_merchant (24)
coop_merchant_type_goods         = 3   # slot_town_merchant (23)
coop_merchant_type_village_elder = 4   # slot_town_elder (25)

# Center type constants (sent in center_encounter event)
coop_center_type_town    = 0
coop_center_type_castle  = 1
coop_center_type_village = 2

########################################################
##  COOP BATTLE CONSTANTS   ############################
########################################################

# Battle types (what kind of encounter triggered the battle)
...
# Battle states (lifecycle of a battle on the battle server)
...
coop_battle_state_abandoned            = 6
# Battle server pool: slot s listens on coop_battle_port_base + 2*s
# (7241/7243/7245/7247 -- 7242 stays the DLL IPC port). Must match
# deploy/configs/BattleServer_<s>.txt and src/battle_ipc.h IPC_MAX_BATTLE_SLOTS.
coop_battle_num_slots = 4
coop_battle_port_base = 7241
...
coop_event_no_capture_heroes                      = 16

# Coop network event subtypes (send_to_player)
...
# Coop class selection slots (one per class index, 0-8)
slot_player_coop_class_0_wanted                   = 50
slot_player_coop_class_1_wanted                   = 51
slot_player_coop_class_2_wanted                   = 52
slot_player_coop_class_3_wanted                   = 53
slot_player_coop_class_4_wanted                   = 54
slot_player_coop_class_5_wanted                   = 55
slot_player_coop_class_6_wanted                   = 56
slot_player_coop_class_7_wanted                   = 57
slot_player_coop_class_8_wanted                   = 58

# Agent slot: which SP party spawned this agent (for coop battle party tracking)
slot_agent_coop_spawn_party                       = 27
slot_agent_coop_banner                            = 28

# Coop battle temp parties -- numeric party indices used by the battle server
# Each range holds 40 parties; 4 ranges total = 160 parties reserved
coop_temp_party_enemy_begin                       = 20   # enemy formation parties
coop_temp_casualties_enemy_begin                  = coop_temp_party_enemy_begin + 40
...
coop_def_battle_size                              = 150
coop_max_battle_size                              = 1000

# Reserve slots for siege phases
coop_reserves_hall                                = 20
coop_reserves_street                              = 80

# Coop round types (siege phase progression)
coop_round_battle                                 = 1
...
# Sandstorm and torch chance settings (sent as event subtypes to client)
coop_sandstorm_chance_min                         = 0
coop_sandstorm_chance_def                         = 20
coop_sandstorm_chance_max                         = 101

coop_torch_chance_min                             = 0
coop_torch_chance_def                             = 3
coop_torch_chance_max                             = 101

# Coop event subtypes for send_to_player settings sync
coop_set_key_crouch                               = 69
coop_set_key_crouch_command                       = 70
coop_set_key_stand_command                        = 71
coop_set_g_crouch_speed_limiter                   = 72
coop_set_ai_crouch_mode                           = 73
coop_set_sp_shield_bash                           = 75
coop_set_sp_shield_bash_ai                        = 76
coop_set_setting_use_spearwall                    = 77
coop_set_g_rand_rain_limit                        = 80
coop_set_tom_sand_storm_chance                    = 82
coop_set_tom_use_banners                          = 83
coop_set_tom_bonus_banners                        = 84
coop_set_tom_use_battlefields                     = 85
coop_set_tom_weapon_break                         = 86
coop_set_tom_lance_breaking                       = 87
coop_set_coop_generate_reduction                  = 88
coop_set_historical_banners                       = 96
coop_set_randomize_player_shield                  = 97
coop_set_torch_chance                             = 98
coop_set_camera_mode                              = 99
coop_sand_storm                                   = 100
coop_voiceset                                     = 105
coop_set_archerpos                                = 106
coop_set_ai_mode                                  = 107

# Character persistence events (client -> server)
coop_event_raise_attribute          = 120
coop_event_raise_skill              = 121
coop_event_raise_proficiency        = 122
coop_event_char_creation_bg         = 123  # gender, father, earlylife
coop_event_char_creation_life       = 124  # adulthood, reason
coop_event_map_spawn_choice         = 125  # x, y

# Skill governing attributes (index = skill ID, value = attribute)
# 0=STR, 1=AGI, 2=INT, 3=CHA
skill_governing_attribute = [
    2,  # 0  skl_trade - INT
    3,  # 1  skl_leadership - CHA
    3,  # 2  skl_prisoner_management - CHA
    0,  # 3  (reserved)
    0,  # 4  (reserved)
    0,  # 5  (reserved)
    0,  # 6  (reserved)
    2,  # 7  skl_persuasion - INT
    2,  # 8  skl_engineer - INT
    2,  # 9  skl_first_aid - INT
    2,  # 10 skl_surgery - INT
    2,  # 11 skl_wound_treatment - INT
    3,  # 12 skl_inventory_management - CHA
    2,  # 13 skl_spotting - INT
    2,  # 14 skl_pathfinding - INT
    2,  # 15 skl_tactics - INT
    2,  # 16 skl_tracking - INT
    2,  # 17 skl_trainer - INT
    0,  # 18 (reserved)
    0,  # 19 (reserved)
    0,  # 20 (reserved)
    0,  # 21 (reserved)
    1,  # 22 skl_looting - AGI
    1,  # 23 skl_horse_archery - AGI
    1,  # 24 skl_riding - AGI
    1,  # 25 skl_athletics - AGI
    1,  # 26 skl_shield - AGI
    1,  # 27 skl_weapon_master - AGI
    0,  # 28 (reserved)
    0,  # 29 (reserved)
    0,  # 30 (reserved)
    0,  # 31 (reserved)
    0,  # 32 (reserved)
    0,  # 33 skl_power_draw - STR
    0,  # 34 skl_power_throw - STR
    0,  # 35 skl_power_strike - STR
    0,  # 36 skl_ironflesh - STR
]

# Skills to display on character screen (ID, display name)
char_screen_skills = [
    (36, "Ironflesh"),
    (35, "Power Strike"),
    (33, "Power Draw"),
...
# Scene prop slots for belfry state tracking
slot_scene_prop_open_or_close_slot                = 1
slot_scene_prop_smoke_effect_done                 = 2
slot_scene_prop_number_of_agents_pushing          = 3
slot_scene_prop_next_entry_point_id               = 4
slot_scene_prop_belfry_platform_moved             = 5
