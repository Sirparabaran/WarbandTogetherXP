...
##        ]
##       ),
      ("coop_direct_connect",[],"COOP Direct Connect",
       [
         (start_presentation, "prsnt_coop_server_ip"),
       ]
      ),
      ("go_back",[],"Go back",
       [(change_screen_quit),
...
#          (try_end),
  ...
  (
    ...
    "Your party encounters a hostile force! They have {reg11} troops.",
    "none",
    [
        (set_background_mesh, "mesh_pic_bandits"),
    ...
    [
      ...
      "Fight in Multiplayer Battle!",
      [
        (assign, "$g_coop_encounter_done", 1),
        # Ask the campaign server to allocate a battle-server slot; the
        # ev-10 battle_available reply carries the port and auto-connects us.
        ...
        (display_message, "@Requesting battle server..."),
        (change_screen_return),
      ]),
      ...
      [
          # Notify campaign server we're entering a local fight (marks player busy,
          # server stops party movement on this player until local_fight_result arrives).
          (multiplayer_send_int_to_server, multiplayer_event_multiplayer_campaign_client_events,
              multiplayer_event_multiplayer_campaign_start_local_fight),

          # Reset the kill-tally parties and arm casualty recording for this
          # battle. Our losses (p_coop_local_player_cas) and the XP basis
          # (p_coop_local_enemy_cas) are filled by mt_lead_charge's
          # ti_on_agent_killed_or_wounded trigger during the fight. We do NOT
          # snapshot/read the player party -- it is unreadable on a coop client
          # after the battle (the connection is torn down by debrief time).
          # Dedicated parties avoid the vanilla counter that wipes
          # p_player_casualties at battle end.
          (party_clear, "p_coop_local_player_cas"),
          (party_clear, "p_coop_local_enemy_cas"),
          (assign, "$g_coop_local_cas_recording", 1),

          # SP path: identical consequence chain to mnu_encounter "Charge the enemy"
          # (simplified - skips enctype_fighting_against_village_raid branches that
          # don't apply to coop encounters).
          (assign, "$g_battle_result", 0),
          (assign, "$g_engaged_enemy", 1),
          (call_script, "script_calculate_renown_value"),
          (call_script, "script_calculate_battle_advantage"),
          ...
          # Scene selection - server-authoritative terrain from the encounter event.
          # (Cannot use script_setup_random_scene: it reads p_main_party which is
          # disabled on a coop client.)
          (assign, ":scene", "scn_random_scene"),
          (try_begin),
              (eq, "$g_coop_local_battle_terrain", rt_steppe),
              (assign, ":scene", "scn_random_scene_steppe"),
          (else_try),
              (eq, "$g_coop_local_battle_terrain", rt_plain),
              (assign, ":scene", "scn_random_scene_plain"),
          (else_try),
              (eq, "$g_coop_local_battle_terrain", rt_snow),
              (assign, ":scene", "scn_random_scene_snow"),
          (else_try),
              (eq, "$g_coop_local_battle_terrain", rt_desert),
              (assign, ":scene", "scn_random_scene_desert"),
          (else_try),
              (eq, "$g_coop_local_battle_terrain", rt_steppe_forest),
              (assign, ":scene", "scn_random_scene_steppe_forest"),
          (else_try),
              (eq, "$g_coop_local_battle_terrain", rt_forest),
              (assign, ":scene", "scn_random_scene_plain_forest"),
          (else_try),
              (eq, "$g_coop_local_battle_terrain", rt_snow_forest),
              (assign, ":scene", "scn_random_scene_snow_forest"),
          (else_try),
              (eq, "$g_coop_local_battle_terrain", rt_desert_forest),
              (assign, ":scene", "scn_random_scene_desert_forest"),
          (else_try),
              (eq, "$g_coop_local_battle_terrain", rt_water),
              (assign, ":scene", "scn_water"),
          (else_try),
              (eq, "$g_coop_local_battle_terrain", rt_bridge),
              (assign, ":scene", "scn_random_scene_plain"),
          (try_end),
          (jump_to_scene, ":scene"),

          # After the mission, jump to our local-fight debrief menu so the
          # casualty + XP diff runs and local_fight_result is sent to the server.
  ...
  # --- Coop Center Encounter ---
  ("coop_center_encounter", mnf_enable_hot_keys|mnf_scale_picture,
    "{s10}",
    "none",
    [
        ...
        (try_begin),
            ...
            (str_store_string, s10, "@You have arrived at the town of {s1}.^^What would you like to do?"),
            (set_background_mesh, "mesh_pic_town1"),
        (else_try),
            (eq, "$g_coop_center_type", coop_center_type_castle),
            (str_store_string, s10, "@You have arrived at the castle of {s1}.^^What would you like to do?"),
            (set_background_mesh, "mesh_pic_castle1"),
        (else_try),
            (str_store_string, s10, "@You have arrived at the village of {s1}.^^What would you like to do?"),
            (set_background_mesh, "mesh_pic_village_p"),
        (try_end),
    ],
    [
      ...
      "Recruit volunteers.",
      [
          # Request recruit pool info (count=0 means "give me info")
          (multiplayer_send_3_int_to_server, multiplayer_event_multiplayer_campaign_client_events,
              multiplayer_event_multiplayer_campaign_request_recruit, "$g_coop_center_party", 0),
      ]),

      ("coop_center_village_trade",
      [
          (eq, "$g_coop_center_type", coop_center_type_village),
      ],
      "Buy supplies from the village elder.",
      [
      ...
      [
          ...
          # Use scene_id sent by server in encounter event
      ...
      [
          # Notify server
          (multiplayer_send_2_int_to_server, multiplayer_event_multiplayer_campaign_client_events,
              multiplayer_event_multiplayer_campaign_start_local_visit, coop_center_type_village),
          (assign, "$g_coop_in_local_visit", 1),
          (assign, "$current_town", "$g_coop_center_party"),
          (assign, "$g_encountered_party", "$g_coop_center_party"),
          # Use scene_id sent by server
          (modify_visitors_at_site, "$g_coop_center_scene"),
          (reset_visitors),
      ...
      [
          # Notify server
          (multiplayer_send_2_int_to_server, multiplayer_event_multiplayer_campaign_client_events,
              multiplayer_event_multiplayer_campaign_start_local_visit, coop_center_type_castle),
          (assign, "$g_coop_in_local_visit", 1),
          (assign, "$current_town", "$g_coop_center_party"),
          (assign, "$g_encountered_party", "$g_coop_center_party"),
          # Use scene_id sent by server
      ...
      [
          (assign, "$g_coop_encounter_done", 1),
          # Ask the campaign server to set up a siege battle for this center; the
          # ev-10 battle_available reply carries the port and auto-connects us.
          (assign, "$g_coop_battle_requested", 1),
          (multiplayer_send_2_int_to_server, multiplayer_event_multiplayer_campaign_client_events,
      ...
      "Lead the assault yourself (local).",
      [
          # Server replies with start_siege_local carrying the wall scene;
          # the mission launches from that receive handler.
          (multiplayer_send_2_int_to_server, multiplayer_event_multiplayer_campaign_client_events,
              multiplayer_event_multiplayer_campaign_request_siege_local, "$g_coop_center_party"),
          (display_message, "@Preparing the assault..."),
      ]),
  ...
  # --- Coop Town Trade (merchant picker) ---
  ("coop_town_trade", mnf_enable_hot_keys|mnf_scale_picture,
    "Choose a merchant to trade with:",
    "none",
    [
        (set_background_mesh, "mesh_pic_town1"),
    ],
    [
      ("coop_trade_weaponsmith",
      [],
      "Weaponsmith",
      [
          (multiplayer_send_2_int_to_server, multiplayer_event_multiplayer_campaign_client_events,
              multiplayer_event_multiplayer_campaign_request_trade, coop_merchant_type_weaponsmith),
      ]),
      ...
      [
          (multiplayer_send_2_int_to_server, multiplayer_event_multiplayer_campaign_client_events,
              multiplayer_event_multiplayer_campaign_request_trade, coop_merchant_type_armorer),
      ]),
      ...
      [
          (multiplayer_send_2_int_to_server, multiplayer_event_multiplayer_campaign_client_events,
              multiplayer_event_multiplayer_campaign_request_trade, coop_merchant_type_horse),
      ]),
      ...
      [
          (multiplayer_send_2_int_to_server, multiplayer_event_multiplayer_campaign_client_events,
              multiplayer_event_multiplayer_campaign_request_trade, coop_merchant_type_goods),
      ]),
  ...
  # --- Coop Recruit Volunteers ---
  ("coop_recruit_volunteers", mnf_enable_hot_keys|mnf_scale_picture,
    "{s10}",
    "none",
    [
        (set_background_mesh, "mesh_pic_recruits"),
        (try_begin),
            (gt, "$g_coop_recruit_amount", 0),
            (str_store_troop_name, s1, "$g_coop_recruit_troop"),
            (assign, reg1, "$g_coop_recruit_amount"),
            (assign, reg2, "$g_coop_recruit_cost"),
            (str_store_string, s10, "@There are {reg1} {s1} available for recruitment. Cost per recruit: {reg2} denars.^^How many do you wish to recruit?"),
        (else_try),
            (str_store_string, s10, "@There are no volunteers available here at this time."),
        (try_end),
    ],
    [
      ("coop_recruit_all",
      ...
      [
          (multiplayer_send_3_int_to_server, multiplayer_event_multiplayer_campaign_client_events,
              multiplayer_event_multiplayer_campaign_request_recruit, "$g_coop_center_party", "$g_coop_recruit_amount"),
      ]),

      ("coop_recruit_5",
      [
          (gt, "$g_coop_recruit_amount", 5),
      ],
      "Recruit 5.",
      [
          (multiplayer_send_3_int_to_server, multiplayer_event_multiplayer_campaign_client_events,
              multiplayer_event_multiplayer_campaign_request_recruit, "$g_coop_center_party", 5),
      ]),
      ...
      [
          (multiplayer_send_3_int_to_server, multiplayer_event_multiplayer_campaign_client_events,
              multiplayer_event_multiplayer_campaign_request_recruit, "$g_coop_center_party", 1),
      ]),
  ...
  ("coop_server_ip", 0,
    "Server IP is currently: {s59}^^Campaign: {s59}:7240^Battle: {s59}:7241-7247^^Change IP or close.",
    "none",
    [],
    [
      ("coop_ip_save",
      ...
      "Save IP and close",
      [
        (display_message, "@Server IP saved: {s59}"),
        (change_screen_return),
      ]),
      ...
      "Reset to 127.0.0.1",
      [
        (str_store_string, s59, "@127.0.0.1"),
        (display_message, "@Server IP reset to 127.0.0.1"),
        (change_screen_return),
      ]),
  ...
  # block builds each slot's enemy name (s2..s5) and troop count (reg1..reg4);
  # module_constants.coop_battle_num_slots == 4 items are listed explicitly
  # because Warband game-menu item lists are static.
  ("coop_join_battle", 0,
    "Available battles -- choose one to join:",
    "none",
    [
      (try_begin),
        (eq, "$coop_avail_valid_0", 1),
        (assign, reg1, 0),
        (try_begin),
          (party_is_active, "$coop_avail_enemy_0"),
          (str_store_party_name, s2, "$coop_avail_enemy_0"),
          (party_get_num_companions, reg1, "$coop_avail_enemy_0"),
        (else_try),
          (str_store_string, s2, "@enemy force"),
        (try_end),
      (try_end),
      (try_begin),
        (eq, "$coop_avail_valid_1", 1),
        (assign, reg2, 0),
        (try_begin),
          (party_is_active, "$coop_avail_enemy_1"),
          (str_store_party_name, s3, "$coop_avail_enemy_1"),
          (party_get_num_companions, reg2, "$coop_avail_enemy_1"),
        (else_try),
          (str_store_string, s3, "@enemy force"),
        (try_end),
      (try_end),
      (try_begin),
        (eq, "$coop_avail_valid_2", 1),
        (assign, reg3, 0),
        (try_begin),
          (party_is_active, "$coop_avail_enemy_2"),
          (str_store_party_name, s4, "$coop_avail_enemy_2"),
          (party_get_num_companions, reg3, "$coop_avail_enemy_2"),
        (else_try),
          (str_store_string, s4, "@enemy force"),
        (try_end),
      (try_end),
      (try_begin),
        (eq, "$coop_avail_valid_3", 1),
        (assign, reg4, 0),
        (try_begin),
          (party_is_active, "$coop_avail_enemy_3"),
          (str_store_party_name, s5, "$coop_avail_enemy_3"),
          (party_get_num_companions, reg4, "$coop_avail_enemy_3"),
        (else_try),
          (str_store_string, s5, "@enemy force"),
        (try_end),
      (try_end),
    ],
    [
      ("coop_join_slot_0",
      [(eq, "$coop_avail_valid_0", 1)],
      "Battle vs {s2} ({reg1} enemies)",
      [
        (assign, reg10, "$coop_avail_port_0"),
        (str_store_string, s58, "@{s59}:{reg10}"),
        (display_message, "@Connecting to battle: {s59}:{reg10}"),
        # Defer the actual connect to the game-loop simple trigger
        # ($g_coop_battle_connect_pending) -- multiplayer_connect_to_server
        # bounces to the MP menu if called straight from a menu consequence.
      ...
      ("coop_join_slot_1",
      [(eq, "$coop_avail_valid_1", 1)],
      "Battle vs {s3} ({reg2} enemies)",
      [
        (assign, reg10, "$coop_avail_port_1"),
        (str_store_string, s58, "@{s59}:{reg10}"),
        (display_message, "@Connecting to battle: {s59}:{reg10}"),
        # Defer the actual connect to the game-loop simple trigger
        # ($g_coop_battle_connect_pending) -- multiplayer_connect_to_server
      ...
      ("coop_join_slot_2",
      [(eq, "$coop_avail_valid_2", 1)],
      "Battle vs {s4} ({reg3} enemies)",
      [
        (assign, reg10, "$coop_avail_port_2"),
        (str_store_string, s58, "@{s59}:{reg10}"),
        (display_message, "@Connecting to battle: {s59}:{reg10}"),
        # Defer the actual connect to the game-loop simple trigger
        # ($g_coop_battle_connect_pending) -- multiplayer_connect_to_server
      ...
      ("coop_join_slot_3",
      [(eq, "$coop_avail_valid_3", 1)],
      "Battle vs {s5} ({reg4} enemies)",
      [
        (assign, reg10, "$coop_avail_port_3"),
        (str_store_string, s58, "@{s59}:{reg10}"),
        (display_message, "@Connecting to battle: {s59}:{reg10}"),
        # Defer the actual connect to the game-loop simple trigger
        # ($g_coop_battle_connect_pending) -- multiplayer_connect_to_server
  ...
  # --- Local SP battle debrief (simplified) ---
  # Entered after a local SP battle ends. Counts casualties, sends results to server.
  ("coop_local_battle_debrief", mnf_scale_picture|mnf_disable_all_keys,
    "{s11}",
    "none",
    [
        (set_background_mesh, "mesh_pic_defeat"),
