    ...
    [
      (ti_on_agent_spawn, 0, 0, [],
       [
         ...
         # Coop-client team reassign: force team based on party match.
         # The SP script_agent_reassign_team relies on $players_kingdom and
         # main_party state which isn't valid on coop clients. Do a simple
         # party-id match instead: player's party -> team 1 (attackers),
         # everyone else -> team 0 (defenders/enemies). Only on coop client
         # (game_in_multiplayer_mode true, not dedicated server).
         # gameType forced to 0 (SP) by ASI before mission start,
         # so the SP reassign path runs naturally.
...
##          (agent_get_team, reg3, ":dead_agent_no"),          
        ...
        # Coop local battle: tally THIS battle's casualties into isolated
        # parties. The vanilla count_mission_casualties_from_agents (called at
        # battle end below) clears p_player_casualties and miscategorises on a
        # coop client, so we record into dedicated parties instead. Gated on
        # the recording flag set by the encounter_fight_sp menu.
        (try_begin),
          (eq, "$g_coop_local_cas_recording", 1),
          (ge, ":dead_agent_no", 0),
          (agent_is_human, ":dead_agent_no"),
          (agent_get_troop_id, ":cas_troop_id", ":dead_agent_no"),
          (neg|troop_is_hero, ":cas_troop_id"),
          (try_begin),
            # Our fallen troops: count KILLED only (wounded recover in campaign).
            (agent_is_ally, ":dead_agent_no"),
            (eq, ":is_wounded", 0),
            (party_add_members, "p_coop_local_player_cas", ":cas_troop_id", 1),
          (else_try),
            # Enemies defeated (killed or wounded) -> XP basis.
            (neg|agent_is_ally, ":dead_agent_no"),
            (party_add_members, "p_coop_local_enemy_cas", ":cas_troop_id", 1),
          (try_end),
        (try_end),
      ...
      (3, 0, 0, [],
       [(assign, ":t0", 0),
        ...
        (assign, reg2, ":t_other"),
        (assign, reg3, ":total"),
        (store_mission_timer_a, reg4),
        (assign, reg5, ":sample_team"),
        (display_log_message, "@DBG t={reg4}: t0={reg0} t1={reg1} other={reg2}(ex:{reg5}) alive={reg3}"),
       ]),
...
##       []),
  ...
  # Coop local siege: copy of castle_attack_walls_ladder with the
  # mtef_attackers/mtef_defenders side flags swapped (teams and entry points
  # unchanged). On a coop client the engine binds the CENTER to the attacker
  # side of the encounter, so the vanilla template spawns the player on the
  # walls and the garrison at the ladders. Swapping only the side flags puts
  # the player party (engine "defender") at the ground entry on team 1
  # ($attacker_team -- charge/ladder AI) and the garrison (engine "attacker")
  # on the walls on team 0 ($defender_team -- hold/archer/ammo-refill AI),
  # keeping every common_siege_* trigger and reinforcement entry index valid.
  (
    "coop_castle_attack_walls_ladder",mtf_battle_mode|mtf_synch_inventory,-1,
    "You attack the walls of the castle...",
    [
     ...
     (10,mtef_attackers|mtef_team_0,af_override_horse,aif_start_alarmed,0,[]),
     (11,mtef_attackers|mtef_team_0,af_override_horse,aif_start_alarmed,7,[]),
     (15,mtef_attackers|mtef_team_0,af_override_horse,aif_start_alarmed,0,[]),
     ...
     (45,mtef_attackers|mtef_team_0|mtef_archers_first,af_override_horse,aif_start_alarmed,1,[]),
     (46,mtef_attackers|mtef_team_0|mtef_archers_first,af_override_horse,aif_start_alarmed,1,[]),
     ],
    [
      common_battle_mission_start,
      common_battle_tab_press,
      ...
      (ti_on_agent_killed_or_wounded, 0, 0, [],
       [
        ...
        # Coop local siege: tally THIS battle's casualties into isolated
        # parties (same recording as mt_lead_charge -- see the comment there).
        # Gated on the flag armed by the local-siege launch handler.
        (try_begin),
          (eq, "$g_coop_local_cas_recording", 1),
          (ge, ":dead_agent_no", 0),
          (agent_is_human, ":dead_agent_no"),
  ...
  # Coop local siege, belfry variant: copy of castle_attack_walls_belfry with
  # the same side-flag swap as coop_castle_attack_walls_ladder (see above).
  # The belfry triggers are side-agnostic: they key off $attacker_team (team 1
  # = the player side here), scene props and entry points 50-55, so the player
  # bots get assigned to push the tower and the init trigger moves it from its
  # scene position (at the wall) back to the start entry.
  (
    "coop_castle_attack_walls_belfry",mtf_battle_mode|mtf_synch_inventory,-1,
    "You attack the walls of the castle...",
    [
     (0,mtef_defenders|mtef_team_1,af_override_horse,aif_start_alarmed,12,[]),
     (0,mtef_defenders|mtef_team_1,af_override_horse,aif_start_alarmed,0,[]),
     ...
     (45,mtef_attackers|mtef_team_0|mtef_archers_first,af_override_horse,aif_start_alarmed,1,[]),
     (46,mtef_attackers|mtef_team_0|mtef_archers_first,af_override_horse,aif_start_alarmed,1,[]),
     (47,mtef_attackers|mtef_team_0|mtef_archers_first,af_override_horse,aif_start_alarmed,1,[]),
     ],
    [
      common_battle_mission_start,
      common_battle_tab_press,
