...
| 9 | -- | ~~Pressing Tab after visiting a village center returned to the settlement menu without running any coop cleanup~~ **Fixed 2026-09-10**: same bug class as row 7, a THIRD affected template found after the hall fix (`village_center`, native's own, unmodified until now). Unlike `town_default`/`town_center`/`castle_visit` -- which each gate on `$g_coop_in_local_visit` and call `script_coop_local_visit_exit` before `finish_mission` on Tab -- `village_center`'s own `ti_tab_pressed` only ever handled a `qst_hunt_down_fugitive` quest-state edge case, unconditionally calling `set_trigger_result,1` for everything else. Whatever made Tab still visually return to the settlement menu did so through some other path (unclear which -- possibly `village_center`'s own default not actually being suppressed the way it looks, since this session couldn't fully pin down the exact native semantics), but never through `coop_local_visit_exit`: `local_visit_done` never reached the server, `$g_coop_in_local_visit` stayed stuck at 1, and the row-6 auto-reconnect safety net never got a chance to arm either. Added the same `$g_coop_in_local_visit`-gated branch as the three working templates, ahead of (not replacing) the existing fugitive-quest logic -- native singleplayer behavior for that quest is unchanged. | `module_mission_templates.py` `village_center` |
| 10 | -- | ~~Row 6's reconnect fix worked (client did reconnect after a local fight), but the loot screen still never appeared~~ **Fixed 2026-09-10, second attempt**: row 6 armed the reconnect on `mnu_coop_local_battle_debrief`'s "Continue" button; a follow-up report ("press tab, takes me to the multiplayer lobby list") revealed that menu often never displays at all -- a real mid-fight disconnect can happen right as `finish_mission` fires, and the engine's own post-disconnect navigation preempts the queued `jump_to_menu` entirely. Fixed by moving the arm itself into the mission's own end-of-battle triggers (`common_battle_tab_press`, the retreat confirm, and both win/loss triggers, `module_mission_templates.py`) -- but that exposed the ACTUAL root cause of this row: the debrief menu's init block was the ONLY place that computed win/loss/casualties/XP and wrote the `$g_coop_result_*` globals the ASI relays -- if that menu is never reached, nothing is ever computed, so the reconnect-and-relay pipeline (row 6, `inventory-sync.md` row 16) has nothing to send even once it succeeds. Extracted that computation into a new idempotent `coop_compute_local_fight_result` (`module_coop_scripts.py`, gated on `$g_coop_local_cas_recording` so a second call after the debrief IS reached is a safe no-op) and call it from the same mission-trigger points, right before arming the reconnect -- guaranteed to run regardless of whether the debrief screen itself ever displays. The debrief's own init is now just a one-line delegate to the same script. | `module_coop_scripts.py` `coop_compute_local_fight_result`; `module_mission_templates.py` `common_battle_tab_press`, `lead_charge`'s retreat/win/hero-fallen triggers, `common_battle_check_victory_condition`, `common_siege_check_defeat_condition`; `module_game_menus.py` `mnu_coop_local_battle_debrief` |
